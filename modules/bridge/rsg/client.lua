...
RegisterNetEvent('rsg-inventory:openShop')
AddEventHandler('rsg-inventory:openShop', function(shopData)
    exports.ox_inventory:openInventory('shop', shopData)
end)
...
local function GetOxWeapon()
    local weapon = exports.ox_inventory:getCurrentWeapon()
    if not weapon then return nil, nil end

    local hash = weapon.hash
    local serial = weapon.metadata and weapon.metadata.serial or weapon.serial

    return hash, serial
end

export('rsg-weapons.weaponInHands', function ()
 for k in pairs(weaponInHands) do weaponInHands[k] = nil end

    local hash, serial = GetOxWeapon()
    if hash and serial then
        weaponInHands[hash] = serial
    end

    return weaponInHands
end)

export('rsg-weapons.CheckWeaponSerial', function()
    return GetOxWeapon() -- returns serial, hash
end)

export('rsg-weapons.UsedWeapons', function(serial)
    UsedWeapons[serial] = nil
end)

export('rsg-weapons.GetUsedWeapons', function()
    return UsedWeapons
end)
